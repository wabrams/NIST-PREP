...
#include "sleep.h"
#include "usound/mic.h"
#include "usound/spk.h"
#include "spp_utils.h"
#include "em_usart.h"
#include "em_rtcc.h"
#include "em_prs.h"
...
typedef enum
{
  ROLE_UNKNOWN = -1, ROLE_CLIENT_MASTER = 0, ROLE_SERVER_SLAVE = 1
} role_t;

// SPP service UUID: 4880c12c-fdcb-4077-8920-a450d7f9b907
const uint8 serviceUUID[16] = { 0x07, 0xb9, 0xf9, 0xd7, 0x50, 0xa4, 0x20, 0x89, 0x77, 0x40, 0xcb,
    0xfd, 0x2c, 0xc1, 0x80, 0x48 };

// SPP data UUID: fec26ec4-6d71-4442-9f81-55bc21d658d6
const uint8 charUUID[16] = { 0xd6, 0x58, 0xd6, 0x21, 0xbc, 0x55, 0x81, 0x9f, 0x42, 0x44, 0x71, 0x6d,
    0xc4, 0x6e, 0xc2, 0xfe };
...
static role_t _role = ROLE_UNKNOWN;
...
static void send_spp_data_client()
{
  ...
  do
  {
    result = gecko_cmd_gatt_write_characteristic_value_without_response(_conn_handle, _char_handle,
        len, &sharedCount)->result;
  }
...
void unlinkPRS()
{
  PRS_SourceAsyncSignalSet(RX_OBS_PRS_CHANNEL, PRS_ASYNC_CH_CTRL_SOURCESEL_NONE, _PRS_ASYNC_CH_CTRL_SIGSEL_NONE);
  PRS_SourceAsyncSignalSet(TX_OBS_PRS_CHANNEL, PRS_ASYNC_CH_CTRL_SOURCESEL_NONE, _PRS_ASYNC_CH_CTRL_SIGSEL_NONE);
}
  ...
  while (1)
  {
    ...
    switch (BGLIB_MSG_ID(evt->header))
    {
      ...
      case gecko_evt_le_gap_scan_response_id:
        if (_main_state == SCAN_ADV)
        {
          ...
          if (process_scan_response(&(evt->data.evt_le_gap_scan_response)) > 0)
          {
            ...
            pResp = gecko_cmd_le_gap_connect(evt->data.evt_le_gap_scan_response.address,
            ...
            {
              printLog("gecko_cmd_le_gap_connect failed with code 0x%4.4x\r\n", pResp->result);
            }
      ...
      case gecko_evt_le_connection_opened_id:
        ...
        printLog("Connected\r\n");
        printLog("Role: %s\r\n",
            (evt->data.evt_le_connection_opened.master == 0) ? slave_string : master_string);
...
//	    	    printLog("try to end scan, result: %x\r\n", result);
        ...
        else
        {
          ...
          gecko_cmd_le_connection_set_timing_parameters(_conn_handle, 400, 400, 0, 200, 0, 0xFFFF);
          _role = ROLE_SERVER_SLAVE;
          pdm_start();
          pdm_on = true;
          //TODO: TEST THIS: enable speaker here
//          startSpeaker();
        }
        gecko_cmd_hardware_set_soft_timer(32768 << 2, CHECK_RESET_TIMER, false);
        ...
        switch (_main_state)
        {
          ...
          case FIND_CHAR:
            if (_char_handle > 0)
            {
              ...
              gecko_cmd_gatt_set_characteristic_notification(_conn_handle, _char_handle,
                  gatt_notification);
      ...
      case gecko_evt_gatt_characteristic_value_id:
        if (evt->data.evt_gatt_characteristic_value.characteristic == _char_handle)
        {
          if (evt->data.evt_gatt_server_attribute_value.value.len > 0)
          {
            ...
            sharedCount = evt->data.evt_gatt_server_attribute_value.value.data[0];
            printLog("[CPKT]> SC: %3d, dt: %lu, %lu\r\n", sharedCount, t - prev_rtcc,
                (t - timer0_RTCC));
            prev_rtcc = t;
            if (sharedCount > 20)
            {
      ...
      case gecko_evt_hardware_soft_timer_id:
        ...
        switch (evt->data.evt_hardware_soft_timer.handle)
        {
          ...
          case CHECK_RESET_TIMER:
            printLog("\t[RSET]> checking if reset needed\r\n");
            if (reset_needed)
            {
              gecko_cmd_le_connection_close(_conn_handle);
            }
      ...
      case gecko_evt_gatt_server_attribute_value_id:
      {
        if (evt->data.evt_gatt_server_attribute_value.value.len > 0)
        {
          uint32_t t = RTCC_CounterGet();
          sharedCount = evt->data.evt_gatt_server_attribute_value.value.data[0];
          printLog("[SPKT]> %d, dt: %lu, %lu, %lu\r\n", sharedCount, t, t - prev_rtcc, (t - timer0_RTCC)); //Server Packet
          prev_rtcc = t;
//				if (sharedCount>20) {
//					gecko_cmd_le_connection_close(_conn_handle);
//				} else {
